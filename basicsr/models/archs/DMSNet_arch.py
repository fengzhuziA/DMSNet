...
class GMN(nn.Module):
    def __init__(self, n_feat, kernel_size=3, bias=True):
        ...
        self.Fextractor = FFblock(n_feat)
        self.convF = FFblock(n_feat)

        self.ATTA = nn.Sequential(
        ...
        self.convfh = conv(n_feat, n_feat, kernel_size=1, bias=bias)
        self.convfx = conv(n_feat, n_feat, kernel_size=1, bias=bias)
        
        self.convimg = conv(n_feat, 3, kernel_size, bias=bias)
    ...
    def forward(self, x, x_img,hidden_state=None):
        if hidden_state is None:
            start_input = self.convfx(x)
        else:
            start_input = self.convfh(hidden_state)+self.convfx(x)
            
       
        img = self.convimg(start_input) + x_img
        
        F_scores =self.ATTA(self.conv3_1(img))
        F_feature = self.Fextractor(start_input)
        F_feature = F_feature*F_scores + start_input
        
        M_scores =self.conv3(img)+F_feature
        M_scores =self.ATTB(M_scores)
       
        M_feature = F_feature*M_scores 
...
class Memory_util(nn.Module):
    ...
    def forward(self, q,k,v):
        ...
        return x
class SMNBlock(nn.Module):
    def __init__(self,in_feat,hops=1):
        super(SMNBlock, self).__init__()
...
class Encoder(nn.Module):
    ...
    def __init__(self,  width=16,enc_blk_nums=[]):
        ...
        for num in enc_blk_nums:
            ...
            chan = chan * 2
        ##SNM blocks
        self.SMN1 = SMNBlock(width,)
        self.SMN2 = SMNBlock(width*2)
        self.SMN3 = SMNBlock(width*4)
        self.SMN4 = SMNBlock(width*8)
    def forward(self, x, encoder_outs=None, decoder_outs=None):
        
        enc1 = self.encoders[0](x)
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc1 = enc1 +self.SMN1(enc1,encoder_outs[0],decoder_outs[0])
            
        x = self.downs[0](enc1)

        enc2 = self.encoders[1](x)
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc2 = enc2 + self.SMN2(enc2,encoder_outs[1],decoder_outs[1])

        x = self.downs[1](enc2)

        enc3 = self.encoders[2](x)
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc3 = enc3 + self.SMN3(enc3,encoder_outs[2],decoder_outs[2])
        x = self.downs[2](enc3)
        
        enc4 = self.encoders[3](x)
        if (encoder_outs is not None) and (decoder_outs is not None):
            enc4 = enc4 + self.SMN4(enc4,encoder_outs[3],decoder_outs[3])
...
class DGNet(nn.Module):
    ...
    def __init__(self,  width=16):
        ...
        self.FF = NAFBlock(chan)
        self.pooling = nn.AdaptiveAvgPool2d(1)
        self.linear = conv(chan,chan, kernel_size=3, bias=True)
...
class DMSNetbaseLocal(Local_Base, DMSNet):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        DMSNet.__init__(self, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    net = DMSNet(img_channel=img_channel, width=width, middle_blk_num=middle_blk_num,
                      enc_blk_nums=enc_blks, dec_blk_nums=dec_blks,time_step=time_step)
